...
```json
{
	"format": "noura.browser-backup",
	"version": 1,
	"workspaceId": "<stable workspace ID>",
	"entries": [
		{
			"path": "<relative file path>",
			"base64": "<canonical padded RFC 4648 base64>"
		}
	]
}
```
...
- The importer checks all entry shapes, paths, canonical base64 and aggregate lengths before allocating decoded binary arrays. JSON parsing itself allocates within the bounded input size; this is not a streaming format. Several in-memory copies may coexist.
...
The browser reports that it requested a download, not that you saved it successfully. The browser revokes object URLs after 60 seconds so download handling can start. Backups carry no authenticity signature or checksum; the import service validates malformed canonical files, but a well-formed modified backup is not authenticated.
