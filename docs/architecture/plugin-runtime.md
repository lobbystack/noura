...
- A plugin activates only when its manifest id appears in `enabled_plugins`. The runtime ignores unknown ids in the manifest, so a workspace carrying ecosystem plugins opens on older builds.
